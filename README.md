...
如果只換掉權杖，Claude Code 會偵測到不一致，並悄悄退回成 **API 計費**。`cl use` 會把這兩塊一起改好，所以切換後仍然走你的 Pro / Max 訂閱。過期的權杖也會在切換時自動更新。切換前，若目前帳號的權杖被執行中的 Claude session 更新過（refresh token 會輪替），`cl use` 會先依帳號身分把它寫回當前 profile，避免遺失剛輪替的權杖（即使用 `--force` 也是）。
